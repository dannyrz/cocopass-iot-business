...
public class BatteryInfoPushThread0 extends Thread {
	Logger LOG = Logger.getLogger(BatteryInfoPushThread0.class.getName());
	Gson gson = new Gson();
	...
	public void run() {
		...
		try {
			rabbitMQChannel.basicConsume(Config.BatteryInfoToBookerQueue, false, consumer);
		...
		while (true) {
			...
			try {
				...
				//batteryInfo = gson.fromJson(strData, com.ludong.model.BatteryInfo.class);
				...
				if (batteryInfo.getChargingPortStatus() > 1) {
					...
					rabbitMQChannel.basicAck(deliveryTag, false);
					System.out.println("dont need push ack success");
				}
